    ...
    <React.Fragment>
      <div className={"list-photos row"}>
        {props.galery.images.map((image, index) => {
          ...
          return (
            <div
              className={"list-photos-item col-lg-4 col-md-6"}
              key={`image_galery-${index}`}
            >
              <div
                className={"photo"}
                style={{
                  backgroundImage: `url(${props.settings.mainUrl}/storage/${url})`,
                }}
                onClick={(event) => {
          ...
          <Modal onClose={closeLightbox}>
            <Carousel
              ...
              views={props.galery.images.map((x) => ({
                source: `${props.settings.mainUrl}/storage/${x.replace(
                  /\\/g,
                  "/"
                )}`,
                caption: "",
